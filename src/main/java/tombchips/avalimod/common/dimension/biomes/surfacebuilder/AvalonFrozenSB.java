...
public class AvalonFrozenSB extends SurfaceBuilder<SurfaceBuilderConfig> {
    public AvalonFrozenSB(Codec<SurfaceBuilderConfig> p_i232136_1_) {
        super(p_i232136_1_);
    }
    ...
    public void apply(Random random, IChunk chunk, Biome biome, int x, int z, int terrainHeight, double noise, BlockState defaultBlock, BlockState defaultFluid, int seaLevel, long seed, SurfaceBuilderConfig config) {
        this.buildSurface(random, chunk, biome, x, z, terrainHeight, noise, defaultBlock, ABlocks.AMMONIA_SOURCE.defaultBlockState(), Blocks.SNOW_BLOCK.defaultBlockState(), Blocks.STONE.defaultBlockState(), config.getUnderwaterMaterial(), seaLevel);
    }
